...
document.addEventListener( 'DOMContentLoaded', () => {
	...
	submitButton.addEventListener( 'click', ( e ) => {
		...
		const locationDataAttr =
			submitButton.getAttribute( 'data-location-info' );
		if ( locationDataAttr ) {
			try {
	...
	function geocodeAddress( address ) {
		...
		fetch( `${ wingSubmitData.restUrl }/geocode`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
				'X-WP-Nonce': wingSubmitData.nonce,
			},
	...
	form.addEventListener( 'submit', ( e ) => {
		...
		fetch( `${ wingSubmitData.restUrl }/submit`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
				'X-WP-Nonce': wingSubmitData.nonce,
	...
	function preFillLocationFields( data ) {
		...
		if ( data.takeout ) {
			const takeoutField = document.querySelector(
				'input[name="wing_takeout"]'
			);
			if ( takeoutField ) {
		...
		if ( data.dineIn ) {
			const dineInField = document.querySelector(
				'input[name="wing_dine_in"]'
			);
			if ( dineInField ) {
